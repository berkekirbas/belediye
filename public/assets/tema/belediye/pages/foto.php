...
if(isset($_GET['id']))
{
	$KATSorgu = $db->prepare("SELECT * FROM foto_galeri WHERE seo = ?");
	$KATSorgu->execute(array($_GET['id']));
	if($KATSorgu->rowCount()){
		$KATSonuc = $KATSorgu->fetch(PDO::FETCH_ASSOC);
	}else{
...
{
	$KATSorgu = $db->prepare("SELECT * FROM foto_galeri ORDER BY id ASC");
	$KATSorgu->execute();
	if($KATSorgu->rowCount()){
		$KATSonuc = $KATSorgu->fetch(PDO::FETCH_ASSOC);
	}else{
...
?>
<?php 
$page = @intval($_GET['foto']);
if(!$page) $page = 1;
$ttsorgu = $db->prepare("SELECT COUNT(*) FROM fotograflar WHERE  resimid = ?");
$ttsorgu->execute(array($KATSonuc['id']));
$total = $ttsorgu->fetchColumn();
$limit= $limitayar['foto_sayisi'];
$page_count = ceil($total/$limit);
if($page > $page_count) $page = 1;
$show = $page * $limit - $limit;
$Sorgu = $db->prepare("SELECT * FROM fotograflar WHERE resimid = ? ORDER BY id DESC LIMIT $show,$limit");
$Sorgu->execute(array($KATSonuc['id']));
$islem = $Sorgu->fetchALL(PDO::FETCH_ASSOC);?>
<div class="haber-detaybg d-lg-block">
    <div class="container">
        <div class="row">
			<div class="innerPageHeading">
				<a class="active" href="index.html"><?=@$dil['anasayfa'];?></a> / <?php echo $KATSonuc['adi'];?>
				<div class="geri">
                ...
                <div class="row">	
                    ...
                    <div class="col-md-9 beyaz">
						<div class="innerPageContent">
							<div class="title">
								<h3><?php echo $KATSonuc['adi'];?></h3>
							</div>
							<?php if($KATSorgu->rowCount() != "0"){?>
							<div class="innerPageNewsDetail">
								<?php echo $KATSonuc['aciklama'];?>
								<div class="addthis_inline_share_toolbox_34zm"></div>
								<div  class="innerGalleryDetail">
									<ul>
										<li>
											<a class="grouped_elements" rel="group2" href="<?php echo TEMA;?>/uploads/fotogaleri/kapak/<?php echo $KATSonuc['kapak']; ?>" title="<?php echo $KATSonuc['adi']; ?>">
												<div class="photo"><img src="<?php echo TEMA;?>/uploads/fotogaleri/kapak/kucuk/<?php echo $KATSonuc['kapak']; ?>"></div>
												<div class="figure">
													<div class="content">
										...
										<li>
											<a class="grouped_elements" rel="group2" href="<?php echo TEMA;?>/uploads/fotogaleri/diger/<?php echo $Sonuc['resim']; ?>" title="<?php echo $KATSonuc['adi']; ?>">
												<div class="photo"><img src="<?php echo TEMA;?>/uploads/fotogaleri/diger/kucuk/<?php echo $Sonuc['resim']; ?>"></div>
												<div class="figure">
													<div class="content">
														<i class="icon fa fa-search-plus"></i>
													</div>
							...
							<div class="well" style="margin-bottom: 30px;"><b><?=@$dil['uzgunuz'];?></b>
								...
								<br>
								<br>
								<button type="button" class="btn btn-primary margin-top-10" onclick="window.history.back();"><?=@$dil['onceki_sayfaya_don'];?></button>
							</div>
							<?php }?>
						</div>
						...
						<div class="text-center">
							<div class="st-pagination">
								...
								if($total <= $limitayar['foto_sayisi']){
									...
									if($page > 1){?>
									<?php $previous = $page - 1;?>
								<li><a href="fotosayfa/<?php echo $KATSonuc['seo'];?>/<?php echo $previous;?>.html">«</a></li>
								<?php }else{?>
								...
								<?php }else{?>
								  <li><a href="fotosayfa/<?php echo $KATSonuc['seo'];?>/<?php echo $i; ?>.html"><?php echo $i; ?></a></li>
								<?php }
								}
								}
								if($page != $page_count){?>
								<?php  $next = $page +1;?>
								<li><a href="fotosayfa/<?php echo $KATSonuc['seo'];?>/<?php echo $next; ?>.html">»</a></li>
								<?php }else{?>
								<li><a href="javascript:void(0);">»</a></li>
								<?php }} ?>
								</ul>
